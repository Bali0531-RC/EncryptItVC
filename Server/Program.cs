...
namespace EncryptItVC.Server
{
    ...
    public class VoiceServer
    {
        ...
        private Dictionary<string, IPEndPoint> _clientVoiceEndpoints = new Dictionary<string, IPEndPoint>(); // Track UDP endpoints
        ...
        private async Task HandleLoginAsync(ClientConnection connection, Message message)
        {
            ...
            if (_users.ContainsKey(username) && VerifyPassword(password, _users[username].PasswordHash))
            {
                ...
                // Register UDP endpoint for voice
                if (connection.TcpClient.Client.RemoteEndPoint is IPEndPoint clientEndpoint)
                {
                    var voiceEndpoint = new IPEndPoint(clientEndpoint.Address, _config.Server.Port + 1);
                    lock (_lockObject)
                    {
                        _clientVoiceEndpoints[username] = voiceEndpoint;
                    }
                }
        ...
        private async Task HandleVoiceDataAsync()
        {
            while (_isRunning)
            {
                try
                {
                    ...
                    // Update client endpoint mapping when we receive voice data
                    lock (_lockObject)
                    {
                        // Try to find the client by IP and update their endpoint
                        foreach (var client in _clients)
                        {
                            if (client.TcpClient.Client.RemoteEndPoint is IPEndPoint tcpEndpoint &&
                                tcpEndpoint.Address.Equals(endpoint.Address))
                            {
                                if (!string.IsNullOrEmpty(client.Username))
                                {
                                    _clientVoiceEndpoints[client.Username] = endpoint;
                                }
                                break;
                            }
                        }
                    ...
                    // Broadcast voice data to same channel
        ...
        private async Task BroadcastVoiceDataAsync(byte[] voiceData, IPEndPoint senderEndpoint)
        {
            try
            {
                // Find the sender by their UDP endpoint
                ClientConnection? senderClient = null;
                string? senderUsername = null;
                
                lock (_lockObject)
                {
                    // Find sender by endpoint
                    foreach (var kvp in _clientVoiceEndpoints)
                    {
                        if (kvp.Value.Address.Equals(senderEndpoint.Address))
                        {
                            senderUsername = kvp.Key;
                            senderClient = _clients.FirstOrDefault(c => c.Username == senderUsername);
                            break;
                        }
                    }
                }

                if (senderClient == null || string.IsNullOrEmpty(senderUsername))
                {
                    return; // Unknown sender
                }

                // Check if sender is muted
                if (senderClient.User != null && senderClient.User.IsMuted)
                {
                    return; // Don't broadcast if muted
                }
        ...
        private void DisconnectClient(ClientConnection connection)
        {
            try
            {
                ...
                if (!string.IsNullOrEmpty(connection.Username))
                {
                    lock (_lockObject)
                    {
                        _clientVoiceEndpoints.Remove(connection.Username);
                    }
                }
